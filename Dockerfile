...
RUN mkdir -p /data && apt-get clean && apt-get update -y && apt-get install python2.7 curl -y && curl -O https://bootstrap.pypa.io/get-pip.py && python2.7 get-pip.py && pip install awscli s3cmd && apt-get clean && chmod a+x /s3sync.sh
